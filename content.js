            ...
            if (requiresLogin) {
                ...
                waitForSectionElement().then(() => {
                    ...
                    if(totalWorkDoneTime === "N/A") {
                        ...
                        // 계산 실행
                        const averageResult = calculateAverageDailyWorkTime(todayWorkTime, totalWorkDoneTime, workdoneDayCount);
                        ...
                        const restNeedWorkTimePerDay = (restNeedTime / restEffectiveWeekdays / 60).toFixed(2);
...
function countWeekdaysUntilToday() {
    ...
    for (let day = startDate; day <= today && day <= endDate; day.setDate(day.getDate() + 1)) {
    ...
    for (let day = 1; day <= lastDayOfMonth; day++) {
        ...
        if (dayOfWeek >= 1 && dayOfWeek <= 5) {
            ...
            const holidayFound = holidays.some(holiday => {
                const [holidayDay, holidayDayOfWeek] = [parseInt(holiday), holiday.slice(-1)];
                return holidayDay === day && ["월", "화", "수", "목", "금"].includes(holidayDayOfWeek);
            });
...
function calculateAverageDailyWorkTime(dailyWorkTime, totalWorkDoneTime, weekdaysUntilToday) {
    ...
    if (weekdaysUntilToday === 0) {
        ...
        // 오늘 근무 시간 (시간, 분으로 변환)
        const dailyWorkParts = dailyWorkTime.split("시간").map(item => item.trim()).filter(Boolean);
        const dailyHours = parseInt(dailyWorkParts[0]);
        const dailyMinutes = dailyWorkParts[1] ? parseInt(dailyWorkParts[1].replace("분", "").trim()) : 0;
